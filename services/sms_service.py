...
PHISHING_KEYWORDS = [
    ...
    "revoked", "cancelled", "terminated", "closed"
]
...
OFFICIAL_SHORTCODES = {
    "MASCOM": ["MASCOM", "MASCOM1", "MASCOM2", "26773"],
    "ORANGE": ["ORANGE", "ORANGE1", "26771"],
    "BTC": ["BTC", "BTC1", "26772"],
    "MTN": ["MTN", "MTN1", "26774"],
...
class SMSFeatureExtractor:
    ...
    def extract_features(self, sms):
        ...
        utility_patterns = [
            ...
            r'balance\s+is\s+[\d,]+\.?\d*',  # Simple balance detection
            r'balance:\s+[\d,]+\.?\d*',       # Balance with colon
            r'bundle\s+(?:expires|activated)', # Bundle messages
            r'recharge\s+successful',          # Recharge confirmation
        ]
        ...
        # Special: Check if it's a simple balance message
        if 'balance' in sms and re.search(r'\d+\.?\d*\s*(?:pula|p)', sms, re.IGNORECASE):
            features['has_utility'] = 1
...
def detect_sms_3layer(sms):
    ...
    for brand, keywords in TELECOM_UTILITY.items():
        if brand in sms_text:
            # Check for balance/bundle keywords (safe)
            if any(u in sms_text for u in ['balance', 'bundle', 'data', 'recharge']):
                # Make sure it's not a phishing attempt
    ...
    def detect(self, sms):
        try:
            ...
            if feature_dict.get('has_utility', 0) == 1 and feature_dict.get('phishing_score', 0) == 0:
                confidence = max(confidence * 0.4, 0.05)  # Reduced confidence for utility
